...
// ---- Gérer confirmation / annulation des rendez-vous ----
if (isset($_POST['confirmer']) || isset($_POST['annuler'])) {
    $id_rdv = intval($_POST['id_rdv']);
    $nouveau_statut = isset($_POST['confirmer']) ? 'confirme' : 'annule';
    $stmt = $conn->prepare("UPDATE rendezvous SET statut=? WHERE id_rdv=?");
    $stmt->bind_param("si", $nouveau_statut, $id_rdv);
    $stmt->execute();
    $stmt->close();
...
// ----- RDV en attente -----
$sql_attente = "
    SELECT r.id_rdv, r.date_rdv, r.heure_rdv, r.statut, p.nom, p.prenom
    FROM rendezvous r
    JOIN patients p ON r.id_patient = p.id_patient
...
// ----- Liste des patients (depuis table patients !) -----
$sql_patients = "
    SELECT id_patient, nom, prenom, date_naissance, adresse, telephone, antecedents_medicaux
    FROM patients
    ORDER BY nom ASC
";
...
<div id="sectionPatients" style="display:none;">
    ...
    <table>
        <tr><th>ID</th><th>Nom</th><th>Prénom</th><th>Date Naissance</th><th>Adresse</th><th>Téléphone</th><th>Antécédents</th><th>Actions</th></tr>
        <?php if($result_patients && $result_patients->num_rows > 0): ?>
            ...
            <tr>
                ...
                <td><?= htmlspecialchars($row['antecedents_medicaux']) ?></td>
                <td>
                    <a href="modifier_patient.php?id=<?= intval($row['id_patient']) ?>" class="btn btn-edit">
                        <i class="fa-solid fa-pen"></i> Modifier
                    </a>

                    <a href="patient_consultation.php?id=<?= intval($row['id_patient']) ?>" class="btn btn-view">
